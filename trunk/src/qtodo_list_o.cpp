...
QTodoListIterator::QTodoListIterator(QTodoList* list) : index(0)
{
...
QWidget* QTodoListIterator::goTo(int _index)
{
	index = _index;
	return current();
}
...
int QTodoListIterator::count()
{
	return m_count;
}

int QTodoListIterator::count(QTodoList* list)
{
...
void QTodoList::pasteTodo(QTodoItem* todo)
{
	...
	radio_dialog.add(QTP::add_above,tr("Paste todos above"),TDP_ABOVE);
	radio_dialog.add(QTP::add_below,tr("Paste todos below"),TDP_BELOW);
	...
	if(radio_dialog.exec() == QDialog::Accepted)
	{
		...
		std::vector<QTodoItem*> sub_todos;
		int pos;
		int base_depth = todo->getDepth() - (*QCB::get()->getItems()->begin())->getDepth();
		QTUM::get()->startRecording();
		switch(radio_dialog.selected())
		{
			case TDP_ABOVE:
				...
				{
					QTodoItem* item = new QTodoItem(*(*it));
	...
	if(radio_dialog.exec() == QDialog::Accepted)
	{
		...
		switch(radio_dialog.selected())
		{
			...
			case TDP_ATBOTTOM:
				for(QTodoItemVector::iterator it = QCB::get()->getItems()->begin(); it != QCB::get()->getItems()->end(); ++it)
				{
					insertTodoAtBottom(new QTodoItem(*(*it)));
				}
				QCB::get()->clear();
				jumpToLast();
				break;
...
int QTodoListIterator::currentIndex()
{
	return index;
}
